...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener('DOMContentLoaded', function() {
      ...
      const initSlider = () => {
        const imageList = document.querySelector(".projects .slider-wrapper .image-list");
        const slideButtons = document.querySelectorAll(".projects .slide-button");
    
        // Slide images according to the slide button clicks
        slideButtons.forEach(button => {
            button.addEventListener("click", () => {
                const direction = button.id === "places-slide" ? 1 : -1;
                slideImage(direction);
            });
        });
    
        // Slide images using scroll wheel
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const sliders = document.querySelectorAll(".image-list");
    
        sliders.forEach((slider) => {
            slider.addEventListener("wheel", (event) => {
                event.preventDefault();
                const direction = event.deltaY > 0 ? 1 : -1;
                slideImage(slider, direction);
            });
        });
    
        const slideImage = (slider, direction) => {
    ...
    // Additional code for mobile navigation, menu toggle, social media links, cuisine slider, header animation, and sticky menu is included in the original response.
    




     // Function to update the project and year counts
     const updateCounts = () => {
      // Get the current counts
      let projectsCount = parseInt(document.getElementById("projectsCount2").innerText);
      let yearsCount = parseInt(document.getElementById("yearsCount2").innerText);

      // Update the counts until the desired values are reached
      if (projectsCount < 500) {
          projectsCount += 5; // You can adjust the increment as needed
      }

      if (yearsCount < 100) {
          yearsCount += 1; // You can adjust the increment as needed
      }

      // Update the HTML elements with the new counts
      document.getElementById("projectsCount2").innerText = projectsCount;
      document.getElementById("yearsCount2").innerText = yearsCount;
  };

  // Call the updateCounts function at a specific interval (e.g., every 1 second)
  setInterval(updateCounts, 100); // Adjust the interval as needed